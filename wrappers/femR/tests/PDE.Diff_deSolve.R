...
for(i in 1:length(N)){
  ...
  if(i == 1){
        ...
        filled.contour(x = x.grid$x.mid,
                       y = x.grid$x.mid,
                       y.hat, main="femR order 1",cex.axis = CEX.axis, cex.main=CEX.axis)
        dev.off()
  }
  ###### femR - ORDER 1 ######
...
plot(log2(h), log2(errors.l2$femR_1), col="red", type="b", pch =16, lwd = LWD, lty = 1, cex = CEX,
        ylim = c( min( min(log2(h^3), log2(errors.l2$femR_1)), min(errors.l2$deSolve, errors.l2$femR_2)), 
                  max( max(log2(h^2), log2(errors.l2$femR_1)), max(errors.l2$deSolve, errors.l2$femR_2))),
...
text(log2(h[3]),(log2(h[3]^2)-0.65), TeX("$h^2$"),cex=CEX.axis)
